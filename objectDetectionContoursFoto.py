...
#define trackbar
cv2.createTrackbar('Threshold Value', 'window', int(binaryThreshValue), int(binaryThreshMax), on_change)
...
if __name__ == '__main__':
    ...
    while True:
        ...
        ret, binaryImage2 = cv2.threshold(processingGreyInvCopy, cv2.getTrackbarPos('Threshold Value', 'window'), binaryThreshMax, cv2.THRESH_BINARY)
        ret, binaryImage3 = cv2.threshold(processingGreyInvCopy, cv2.getTrackbarPos('Threshold Value', 'window'), binaryThreshMax, cv2.THRESH_BINARY)
